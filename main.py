...
def main():
    ...
    transform_train = TrainTransforms(in_channels=origin_channels, out_channels=in_channels, size=size,
                                      horizontal_flip=True, random_affine=True, random_erasing=False)
    ...
    # optimizer = torch.optim.RMSprop(net.parameters(), lr=0.002, alpha=0.9, eps=1e-07, momentum=0.1, centered=True)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.25, patience=2, verbose=False, mode='min', threshold=0.0001,
    ...
    # fig = make_image_label_figure(images, labels, class_names=MNIST_CLASSES)
    # logger.add_figure('Train Images', fig)
    logger = TensorBoardLogger(logdir, class_names=VOC2012_CLASSES)
    trainer = PyTorchTrainer(device=device, epoch_callback=logger.epoch_callback, batch_callback=logger.batch_callback)
    trainer.train(net, optimizer, criterion, trainloader, valloader, logdir, scheduler=scheduler, epochs=60)
